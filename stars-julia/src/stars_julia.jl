...
function dot(l::Vec3{T}, r::Vec3{T})::T where T
...
function rand_unit_float64!(xs::RandState)::Float64
    return Float64(rand_uint64!(xs)) / Float64(0xFFFF_FFFF_FFFF_FFFF)
end

function rand_unit_float32!(xs::RandState)::Float32
    return Float32(rand_unit_float64!(xs))
end

function rand_unit_vec3!(xs::RandState)::Vec3{Float32}
    return from_spherical(
...
function timer_new_fps(timer::TimeCounter)::Union{Float32, Nothing}
    return if timer.now == timer.fps_last_measure
        timer.fps :: Union{Float32, Nothing}
    else
        nothing :: Union{Float32, Nothing}
    end
end
...
function input_from_key(k::SDL_Scancode, pressed::Bool)::Input
    ks = pressed ? +1 : -1
    return if k == SDL_SCANCODE_Q Input(rot = +ks)
...
function move_stars!(context::Context, delta::Vec3{Float32})
    for i in eachindex(context.stars)
        star = context.stars[i] + delta
        context.stars[i] = if dot(star, star) < 1
            star
        else
            star = rand_unit_vec3!(context.rand)
            dot(star, delta) >= 0 ? -star : star
        end
    end
end
...
function c_arrow(p::Ptr{T}, s::Symbol) where {T}
    ...
    if i === nothing
        throw(ArgumentError("$T does not have field $s"))
    end
...
function render!(context::Context)
    ...
    if surface_ptr == C_NULL
        println("Cannot lock window surface: $(unsafe_string(SDL_GetError()))")
        return
    end

    pixel_format_ptr = c_arrow(surface_ptr, :format)
    bpp = c_arrow(pixel_format_ptr, :BytesPerPixel)
    if bpp != 4
        println("Window surface must have BPP of 4")
        return
    end

    if SDL_LockSurface(surface_ptr) != 0
        println("Failed locking window surface: $(unsafe_string(SDL_GetError()))")
        return
    end
    ...
    whalf::Float32 = w / 2.0f0
    hhalf::Float32 = h / 2.0f0
    clip::Float32 = 0.5f0
    wh_scale = sqrt(Float32(w * w + h * h) / (1 - clip * clip))
    xymul::Float32 = clip * w * h / wh_scale

    # Project stars and sort them
    proj_stars::Vector{ProjStar} = []
    for s::Vec3{Float32} in context.stars
        if s.z <= 0 continue end
        xs = Base.unsafe_trunc(Int32, whalf + xymul * s.x / s.z)
        if xs < 0 || xs >= w - 4 continue end
        ys = Base.unsafe_trunc(Int32, hhalf - xymul * s.y / s.z)
        if ys < 0 || ys >= h - 4 continue end
        push!(proj_stars, ProjStar(xs, ys, dot(s, s)))
    end
    proj_stars = sort(proj_stars, lt = (l, r) -> l.d2 > r.d2)
...
function run!(context::Context)
    ...
    while !do_quit
        ...
        dr = Vec3(context.input.dx * ds, context.input.dy * ds, ds)
        move_stars!(context, dr)
...
function (@main)(_)
    @assert SDL_Init(SDL_INIT_VIDEO) == 0 "SDL2 init failed: $(unsafe_string(SDL_GetError()))"
    ...
    @assert window != C_NULL "Failed to create window"
